...
const GroupCard: React.FC<Props> = ({ group, onGroupChange, onAddItem, onDeleteItem, onDeleteGroup, onGroupNameChange, editable = true }) => {
  const gtype = groupTypeConfig[group.groupType] || { label: group.groupType, color: 'default' };
  ...
  return (
    <Card
      ...
      style={{
        ...
        borderTop: `3px solid ${group.isFixed ? COLORS.primary : COLORS.success}`,
      }}
      title={
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, width: '100%' }}>
          <Tag color={gtype.color} style={{ minWidth: 64, textAlign: 'center' }}>{gtype.label}</Tag>
          {!group.isFixed && onGroupNameChange && (
            editingName ? (
          ...
          {group.isFixed && <Tag style={{ fontSize: 11 }}>固定</Tag>}
          <Badge
            count={`¥${formatMoney(summary.totalAccountingPrice)}`}
