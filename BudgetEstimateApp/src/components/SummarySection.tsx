  ...
  ({ value, label, onClick }) => (
    <span onClick={onClick} title={onClick ? "点击切换" : undefined}
      style={{
        ...
        background: onClick ? COLORS.bgSelected : 'transparent',
        color: onClick ? COLORS.primary : COLORS.textSecondary,
        fontSize: 12, fontWeight: 600, userSelect: 'none',
        transition: 'background 0.12s',
      }}
      onMouseEnter={e => { if (onClick) e.currentTarget.style.background = '#dce6f5'; }}
      onMouseLeave={e => { if (onClick) e.currentTarget.style.background = COLORS.bgSelected; }}
    >
    ...
    >
      ...
      <div style={{ display: 'flex', gap: 12, marginBottom: 20 }}>
        ...
        <div style={{
          ...
          <span style={{ fontWeight: 700, fontSize: 15, color: COLORS.textDark }}>¥{formatMoney(summary.warrantyBase)}</span>
          <span style={{ color: COLORS.textLight, fontSize: 12 }}>×</span>
          <PctBadge value={warnPct} label="系数"
            onClick={readOnly ? undefined : () => { const n = WARNS[(warnPct + 1) % WARNS.length]; onVersionUpdate?.('warrantyRate', n / 100); }} />
          <span style={{ color: COLORS.textLight, fontSize: 12 }}>=</span>
          <span style={{ fontWeight: 700, fontSize: 16, color: COLORS.primary, whiteSpace: 'nowrap' }}>
            ¥{formatMoney(summary.warrantyCost)}
          </span>
        </div>
        ...
        <div style={{
          ...
          <PctBadge value={riskPct} label="系数"
            onClick={readOnly ? undefined : () => { const n = WARNS[(riskPct + 1) % WARNS.length]; onVersionUpdate?.('riskRate', n / 100); }} />
          <span style={{ color: COLORS.textLight, fontSize: 12 }}>=</span>
          <span style={{ fontWeight: 700, fontSize: 16, color: COLORS.warning, whiteSpace: 'nowrap' }}>
            ¥{formatMoney(summary.riskCost)}
          </span>
        </div>
        ...
        <div style={{
          ...
          {readOnly ? (
            <span style={{ fontWeight: 700, fontSize: 16, color: COLORS.purple }}>¥{formatMoney(version.commercialCost)}</span>
          ) : (
          <input type="text" inputMode="numeric"
            key={'com-cost-' + version.commercialCost}
        ...
        <Col span={8}>
          <div style={{
            ...
            <div style={{ marginTop: 8, fontSize: 12, color: COLORS.textSecondary }}>
              欧元 <span style={{ fontWeight: 600, color: COLORS.textDark }}>€{formatMoney(Math.round(Math.round(summary.totalAccountingPrice / (1 + (version.taxRate ?? TAX_RATE))) / version.eurRate))}</span>
            </div>
          </div>
        </Col>
        <Col span={8}>
          <div style={{
            ...
            <div style={{ marginTop: 8, fontSize: 12 }}>
              ...
              {Math.abs(summary.discountRate) < 0.001 ? (
                <span style={{ fontWeight: 700, color: COLORS.textLight }}>—</span>
              ) : (
                <span style={{
                  fontWeight: 700,
                  color: summary.discountRate > 0 ? '#f5222d' : COLORS.success,
                }}>
                  {summary.discountRate > 0 ? '-' : '+'}{(Math.abs(summary.discountRate) * 100).toFixed(1)}%
                </span>
              )}
            </div>
          </div>
        ...
        <Col span={8}>
          <div style={{
            ...
            <div style={{ fontWeight: 700, fontSize: 24, lineHeight: 1.2 }}>
              ¥{formatMoney(Math.round(summary.discountedPrice - summary.totalCost * (1 + (version.taxRate ?? TAX_RATE))))}
            </div>
